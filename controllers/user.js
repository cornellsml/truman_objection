...
exports.createGuest = (req, res, next) => {
    ...
    User.findOne({ r_id: req.query.r_id }, (err, existingUser) => {
        ...
        user.save((err) => {
            ...
            // generating a session for the user -- Not necessary for a one-shot study
            // req.logIn(user, (err) => {
            //     if (err) {
            //         return next(err);
            //     }
...
exports.postPageLog = (req, res, next) => {
    User.findOne({ r_id: req.query.r_id }, (err, user) => {
        ...
        if (!user) {
            req.flash('errors', {
                msg: 'User doesnt exist, which means they didnt see the home page.'
            });
            res.set('Content-Type', 'application/json; charset=UTF-8');
            return res.send({ result: "no success" });
        }
        user.logPage(Date.now(), req.body.page, req.body.search);
        user.save((err) => {
            if (err) {
                return next(err);
            }
 ...
 * Get video feed
 */
exports.getTestFeed2 = (req, res, next) => {
    User.findOne({ r_id: req.query.r_id }, (err, user) => {
        if (err) { return next(err); }
        if (!user) {
            // happens when no r_id is provided, or the user didn't see home page + profile page
 ...
 * Get video feed
 */
exports.getTestFeed5 = (req, res, next) => {
    User.findOne({ r_id: req.query.r_id }, (err, user) => {
        if (err) { return next(err); }
        if (!user) {
            // happens when no r_id is provided, or the user didn't see home page + profile page
 ...
 * Get video feed
 */
exports.getTestFeed6 = (req, res, next) => {
    User.findOne({ r_id: req.query.r_id }, (err, user) => {
        if (err) { return next(err); }
        if (!user) {
            // happens when no r_id is provided, or the user didn't see home page + profile page
...
exports.postAction = (req, res, next) => {
    User.findOne({ r_id: req.body.r_id }, (err, user) => {
        ...
        if (!user) {
            // User doesn't exist: go to home page. Should never happen, but if it does
            return res.redirect((req.query.r_id) ? '/?r_id=' + req.query.r_id : '/');
        }

        let userAction = user.feedAction;

        const condition = parseInt([req.body.off_id, req.body.obj_t_id, req.body.obj_m_id].join(''));
        // Find the object from the corresponding experimental condition
        let experimentalIndex = _.findIndex(userAction, function(obj) {
            return obj.condition_id === condition;
        });

        if (experimentalIndex == -1) {
            // Condition does not exist yet in User DB, so we have to add it now
            user.conditions.push(condition);

            let cat = {};
            cat.condition_id = condition;
            cat.off_id = req.body.off_id;
            cat.obj_t_id = req.body.obj_t_id;
            cat.obj_m_id = req.body.obj_m_id;
            cat.videoAction = {};
            cat.commentAction = [];

            experimentalIndex = userAction.push(cat) - 1;
        }

        // userAction is the correct user.feedAction array
        // we found the right condition
        // and experimentalIndex is the correct index for that condition in the action array

        // create a new comment
        if (req.body.new_comment) {
            const cat = {};
            cat.new_comment = true;
            cat.commentID = user.numComments + 6; // user-made comments begins at 6
            user.numComments = user.numComments + 1;
            cat.new_comment_time = Date.now();
            cat.comment_body = req.body.comment_body;

            if (req.body.reply_to) {
                cat.reply_to = req.body.reply_to;
            }
            userAction[experimentalIndex].commentAction.push(cat);
        }
        // like, dislike, flag, share a comment or video
        else {
            let updateValue;
            // Determine if action was conducted on a comment or a video
            if (req.body.commentID) {
                let commentIndex = _.findIndex(userAction[experimentalIndex].commentAction, function(o) {
                    return o.commentID == req.body.commentID;
                });

                // no comment in this post-actions yet: only happens if action is done on an actor comment
                if (commentIndex == -1) {
                    const cat = {};
                    cat.commentID = req.body.commentID;
                    commentIndex = userAction[experimentalIndex].commentAction.push(cat) - 1;
                }
                updateValue = userAction[experimentalIndex].commentAction[commentIndex];
            } else {
                updateValue = userAction[experimentalIndex].videoAction;
            }

            // LIKE, DISLIKE, FLAG, SHARE
            updateValue[req.body.actionType].action = req.body.actionValue;

            // Special scenario where liking and disliking are mutually exclusive
            if (req.body.actionType == "liked" && req.body.actionValue == "true") {
                updateValue["disliked"].action = false;
            }
            if (req.body.actionType == "disliked" && req.body.actionValue == "true") {
                updateValue["liked"].action = false;
            }

            if (updateValue[req.body.actionType].absTimes) {
                // not the first action
                updateValue[req.body.actionType].absTimes.push(Date.now());
            } else {
                // this is the first action 
                updateValue[req.body.actionType].absTimes = [Date.now()];
            }
        }

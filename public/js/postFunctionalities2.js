...
function likePost(e) {
    ...
    if (icon.hasClass("green")) {
        ...
        const label = target.siblings("a.ui.basic.green.right.pointing.label");
        ...
        icon.addClass("green");
        var label = target.siblings("a.ui.basic.green.right.pointing.label");
        label.html(function(i, val) { return val * 1 + 1 });

        let dislike = $('.ui.downvote.button');
        if (dislike.find('i').hasClass("red")) {
            dislike.find('i').removeClass("red");
...
function dislikePost(e) {
    ...
    if (icon.hasClass("red")) {
        ...
        if (like.find('i').hasClass("green")) {
            like.find('i').removeClass("green");
            var label = like.siblings("a.ui.basic.green.right.pointing.label");
            label.html(function(i, val) { return val * 1 - 1 });
        }
        $.post("/feed", {
            off_id: off_id,
